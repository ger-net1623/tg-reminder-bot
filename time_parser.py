...
# "в 10" → "в 10:00". dateparser сам по себе не распознаёт час без минут
# в русском, а пользователи так пишут постоянно ("завтра в 10").
# Срабатывает только когда после числа НЕТ двоеточия или ещё одной цифры
# (чтобы не трогать "в 10:30", "в 2026" и т.п.).
_HOUR_ONLY_RE = re.compile(r"\bв\s+(\d{1,2})\b(?![:\d])", re.IGNORECASE)
...
def _normalize_input(text: str) -> str:
    ...
    text = _HOUR_ONLY_RE.sub(r"в \1:00", text)
    return text
...
if __name__ == "__main__":
    ...
    cases = [
        "завтра в 19:00",
        "завтра в 10",        # час без минут — должен стать 10:00
        "сегодня в 23:30",
        "сегодня в 23",       # то же — 23:00
        "через 30 минут",
        "через 2 часа",
        "11.05 19:00",
        "12.05.2026 09:30",
        "в пятницу в 18:00",
        "в пятницу в 9",      # пятница 09:00
